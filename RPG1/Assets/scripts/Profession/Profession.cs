...
public class Profession : MonoBehaviour
{
    ...
    public void ShowDescription(Recipe recipe) //seçilen tarifi seçme için
    {
    ...
    private bool CanCraft() //nugget altınlarımın sayısı büyükse büyük altın yapan fonksiyon
    {
        bool canCraft = true;
    ...
    public void AddItemsToInventory()
    {
        if (InventoryScript.MyInstance.AddItem(craftItemInfo.MyItem))
        {
            foreach (CraftingMaterial material in selectedRecipe.Materials)
            {
                ...
                {
                    InventoryScript.MyInstance.RemoveItem(material.MyItem); //tamamladığım altını inventoryden kaldırıp büyük altını göstermek için
                }
            }
        }
